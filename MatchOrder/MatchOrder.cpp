...
int MatchOrder::matchOrders(std::vector<Order::Order>& buyOrders, std::vector<Order::Order>& sellOrders, std::vector<Order::Order>& tradeHistory) {
    ...
    while (buyIt != buyOrders.end() && sellIt != sellOrders.end()) {
        ...
        Utils::Logger::getInstance().log("Evaluating orders for matching...");
        Utils::Logger::getInstance().log("Buy Order: ID " + boost::uuids::to_string(buyIt->id) + ", Price: " + std::to_string(buyIt->price) + ", Quantity: " + std::to_string(buyIt->quantity));
        Utils::Logger::getInstance().log("Sell Order: ID " + boost::uuids::to_string(sellIt->id) + ", Price: " + std::to_string(sellIt->price) + ", Quantity: " + std::to_string(sellIt->quantity));
        ...
        if (buyIt->type == OrderType::MARKET_BUY || sellIt->type == OrderType::MARKET_SELL) {
            ...
            Utils::Logger::getInstance().log("Market trade executed: " + std::to_string(tradeQuantity) + " @ " + std::to_string(tradePrice));
        ...
        // Handle limit orders
            ...
            int tradeQuantity = std::min(buyIt->quantity, sellIt->quantity);
            Utils::Logger::getInstance().log("Limit trade executed: " + std::to_string(tradeQuantity) + " @ " + std::to_string(sellIt->price));
    ...
    Utils::Logger::getInstance().log("Order matching completed.");
    return matched;
}
